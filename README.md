# Cameron Goodwin

*Cloud & Network Engineering Student | Seeking IT Support & Cloud Support Roles*

[![LinkedIn](https://img.shields.io/badge/LinkedIn-cameron--goodwin-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/camerongoodwin03)

---

> **Portfolio Note**
>
> The projects in this portfolio are designed to **demonstrate hands-on experience with the tools and workflows used daily in enterprise IT support and cloud support environments**.
>
> Each project focuses on practical tasks such as user lifecycle management, identity troubleshooting, IT service management, and Microsoft Azure administration — the same work performed by IT Support Specialists, M365 Administrators, and Cloud Support Engineers in enterprise organizations.
>
> The goal is to show not just that I studied these tools, but that I can **deploy, configure, troubleshoot, and document them at an enterprise level**.

---

## Projects

| Project | Proof | Purpose | Stack | Status |
| --- | --- | --- | --- | --- |
| [**Hybrid Identity & Azure Infrastructure Lab**](https://github.com/camerongoodwin03/hybrid-identity-lab) | Screenshots Included | Deployed enterprise Active Directory on Azure, configured hybrid identity sync via Microsoft Entra Connect, enforced Zero Trust security controls through Conditional Access and MFA, and completed 4 real-world IT support troubleshooting scenarios including sync failure, user access revocation, MFA onboarding, and license management. | Azure · Windows Server · AD DS · Entra ID · M365 · PowerShell | ![Complete](https://img.shields.io/badge/Complete-2ea44f?style=flat-square) |
| **PowerShell Automation & M365 Administration Lab** | | Automating user lifecycle workflows in PowerShell — account creation, M365 license assignment, mailbox provisioning, and account deprovisioning. Managing Entra ID authentication, MFA, and Exchange Online distribution groups. | PowerShell · M365 E5 · Exchange Online · Entra ID | ![Complete](https://img.shields.io/badge/Complete-2ea44f?style=flat-square) |
| **IT Service Management & Ticketing Lab** | | Deploying osTicket on an Azure VM and documenting a full enterprise incident lifecycle — ticket creation, triage, escalation, resolution, and closure — aligned to ITIL 4 service management principles. | osTicket · Azure VM · ITIL 4 | ![Queued](https://img.shields.io/badge/Queued-888888?style=flat-square) |

---

## Technical Skills

| Category | Skills |
| --- | --- |
| **Cloud & Azure** | Microsoft Azure · Azure VMs · Azure VNets · Azure RBAC · Resource Groups |
| **Identity & Access** | Microsoft Entra ID · Active Directory · Azure AD Connect · MFA · Conditional Access |
| **M365 Administration** | Microsoft 365 · Exchange Online · License Management · User Provisioning · Onboarding/Offboarding |
| **Automation** | PowerShell · User lifecycle automation · ADSync management |
| **IT Support** | ITIL 4 · Incident Documentation · Root Cause Analysis · Technical Troubleshooting |

---

## Education

**B.S. Cloud and Network Engineering** — Western Governors University *(In Progress)*

---

[Connect on LinkedIn](https://www.linkedin.com/in/camerongoodwin03)
